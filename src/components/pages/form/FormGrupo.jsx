...
//import BCPQR from './vite.svg';
import YapeQR from './yapeqr.png';
...
// ... (mantener los componentes FormInput, CategorySelect, PaymentMethod, FileInput y ResultDialog igual)

const TeamMembersTable = ({ members, setMembers, error, helperText }) => {
  const [newMember, setNewMember] = useState({ name: '', dni: '' });
  const [memberErrors, setMemberErrors] = useState({ name: '', dni: '' });

  const validateMember = () => {
    const errors = {};
    if (!newMember.name.trim()) errors.name = 'El nombre es requerido';
    if (!newMember.dni.trim()) {
      errors.dni = 'El DNI es requerido';
    } else if (!/^\d{8}$/.test(newMember.dni)) {
  ...
  const handleAddMember = () => {
    if (validateMember()) {
      ...
      setNewMember({ name: '', dni: '' });
      setMemberErrors({ name: '', dni: '' });
    }
  };
    ...
    <Box className="mb-6">
        ...
        <Box className="flex flex-col gap-4 md:flex-row">
          <TextField
            ...
            onChange={(e) => {
              ...
              setMemberErrors({ ...memberErrors, name: '' });
            }}
          ...
          <TextField
            ...
            onChange={(e) => {
              ...
              setMemberErrors({ ...memberErrors, dni: '' });
            }}
            error={!!memberErrors.dni}
            helperText={memberErrors.dni}
      ...
      <TableContainer component={Paper}>
        <Table>
          <TableHead>
            <TableRow>
              <TableCell className="whitespace-normal">Nombre y Apellido</TableCell>
              <TableCell className="whitespace-normal">DNI</TableCell>
              <TableCell align="right" className="whitespace-normal">Acciones</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {members.map((member, index) => (
              <TableRow key={index}>
                <TableCell className="whitespace-normal break-words">{member.name}</TableCell>
                <TableCell className="whitespace-normal break-words">{member.dni}</TableCell>
...
const RegistrationForm = ({ categories }) => {
  ...
  const handleFileChange = (e) => {
    ...
    if (teamMembers.length === 0) {
      newErrors.teamMembers = 'Debe agregar al menos un integrante';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Agregar los integrantes al FormData
      formDataToSend.append('teamMembers', JSON.stringify(teamMembers));

      const response = await fetch('./register_grupal.php', {
        method: 'POST',
        body: formDataToSend,
      });
  ...
  return (
    ...
    >
      <Paper elevation={3} sx={{ p: 4, borderRadius: 2, bgcolor: '#ffffff' }}>
        {/* Mantener los FormInput existentes */}
        <FormInput
        ...
        {/* ... (resto de los FormInput) */}

        {/* Agregar el componente TeamMembersTable después de los datos del representante */}
        <TeamMembersTable
          members={teamMembers}
        ...
        {/* Mantener el resto de los componentes */}
        <CategorySelect
...
export const Principal = () => {
  const categories = [
    { id: 'SOCCER 16-', name: 'SOCCER MENORES DE 16', price: 30 },
    { id: 'SOCCER 17+', name: 'SOCCER MAYORES DE 17', price: 30 },
  ];
      ...
      <div className="scroll-container">
        ...
        >
          ...
          >
            ...
            Registro de Competencia Grupal
          </Typography>
          <Box>
